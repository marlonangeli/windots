...
{
    if ($global:__TerminalIconsLoaded)
    { return
    }
...
$global:__WorktrunkInitialized = $false
function Enable-Worktrunk
{
    if ($global:__WorktrunkInitialized)
    { return $true
    }
    if (-not (Get-Command git-wt -ErrorAction SilentlyContinue))
    { return $false
    }

    if (Invoke-ShellInitScript -Command "git-wt" -Arguments @("config", "shell", "init", "powershell"))
    {
        $global:__WorktrunkInitialized = $true
        return $true
    }
...
{
    if ($global:__MiseCompletionInitialized)
    { return $true
    }
